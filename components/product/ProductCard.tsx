  ...
  return (
    <div className="flex flex-shrink-0 w-[260px] group flex-col gap-y-4 relative">
      ...
      { ((product.pricelist && product.offlinePrice) && ((product.pricelist!.trim() && product.offlinePrice.trim()) 
        ...
        <span className="absolute bottom-[10px] left-[10px] z-[15] text-[11px] bg-red-600 text-white px-3 py-[2px] rounded-sm font-bold">-{Math.round(((parseInt(product.pricelist!)-parseInt(product.offlinePrice))/parseInt(product.pricelist!))*100)}%</span>
      }
