...
// ─── CONSTANTS ─────────────────────────────────────────────────────────────────
const BRANDS = ["Samsung", "Sharp", "LG", "Panasonic", "Sony", "Daikin", "Aqua", "Polytron", "Toshiba", "Lainnya"];
const CATEGORIES = ["Elektronik", "AC & Pendingin", "Mesin Cuci", "Kulkas & Freezer", "Furnitur", "Peralatan Dapur", "Lainnya"];
...
export default function UploadProduk() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    const payload = { ...data, images: images.map((img) => img.url) };
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 flex flex-col gap-5">
          ...
          <SectionCard icon={Package} title="Informasi Produk" subtitle="Lengkapi info dasar produk">
            <div className="flex flex-col gap-4 px-5 py-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="URL / SKU" hint="Opsional — slug atau kode internal">
                  <input
                    {...register("url")}
          ...
          <SectionCard icon={Ruler} title="Dimensi & Berat" subtitle="Untuk kalkulasi ongkir yang akurat">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <Field label="Berat" required error={errors.berat}>
                <UnitInput
                  unit="gram"
                  error={errors.berat}
                  placeholder="0"
                  {...register("berat", {
                    required: { value: true, message: "Berat wajib diisi" },
                  })}
                />
              </Field>
              <Field label="Panjang">
